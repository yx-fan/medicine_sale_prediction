...
# 加载已处理的组合
already_processed = pd.read_csv('v2_xgboost_best_model_results_v4_already.csv')
already_processed['药品名称厂家组合'] = already_processed['药品名称'] + '_' + already_processed['厂家']
df['药品名称厂家组合'] = df['药品名称'] + '_' + df['厂家']

# 过滤掉已处理的组合
df = df[~df['药品名称厂家组合'].isin(already_processed['药品名称厂家组合'])]
...
for _, row in unique_groups.iterrows():
    ...
    # Plot predictions
    plot_predictions(group_data.set_index('ds'), rolling_predictions, drug_name, factory_name, configmonth.font_path, "model_plots_xgv4auto_v2")
