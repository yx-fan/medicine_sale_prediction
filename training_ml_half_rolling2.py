...
def deal_with_outlier(df):
    ...
    对于超过组内平均值2.5倍或低于0.4倍的值，将其替换为过去3期数据的平均值。
    ...
    def replace_outliers(group):
        # 计算当前分组的中位数值
        median_value = group['减少数量'].median()
        # 设置异常值的上下限
        upper_limit = median_value * 2
        lower_limit = median_value * 0.5
...
for _, row in unique_groups.iterrows():
    ...
    # 构建外生变量   #试着看去掉外生变量会不会好一点 20241113
    exog_cols = ['期初金额占比', 'previous_增加数量'] #+ [f'avg_6_period_sales_{other_factory}' for other_factory in other_factories['厂家'].unique()] #+ month_cols
    ...
    if use_rolling_forecast:
        ...
        for t in range(train_end, len(log_y)):
            ...
            try:
                next_pred_log = auto_model.predict(n_periods=1, exogenous=exog_current).item()
                # 添加异常值检查逻辑 20241113
                median_value = log_y[:t].median()
                upper_limit = median_value * 2  # 上限为组内平均值的2.5倍
                lower_limit = median_value * 0.5  # 下限为组内平均值的0.4倍
                # 检查预测值是否在合理范围内
                if next_pred_log > upper_limit or next_pred_log < lower_limit:
                    # 如果超出上下限，替换为过去3期数据的平均值
        ...
        for t in range(train_end, len(log_y)):
            ...
            try:
                ...
                # 添加异常值检查逻辑 20241113
                median_value = log_y[:t].median()
                upper_limit = median_value * 2.5  # 上限为组内平均值的2.5倍
                lower_limit = median_value * 0.4  # 下限为组内平均值的0.4倍
                # 检查预测值是否在合理范围内
                if next_pred_log > upper_limit or next_pred_log < lower_limit:
                    # 如果超出上下限，替换为过去3期数据的平均值
                    next_pred_log = log_y[max(0, t-3):t].mean()
