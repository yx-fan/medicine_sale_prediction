...
parser.add_argument('--drug_name', type=str, help='Specify a single drug name to process')  # 添加药品名称参数
...
# 如果指定了药品名称，则只处理该药品
if args.drug_name:
    df = df[df['药品名称'] == args.drug_name]
    if df.empty:
        print(f"Error: No data found for the specified drug name: {args.drug_name}")
        exit(1)
...
for _, row in unique_groups.iterrows():
    ...
    print("rolling: ", rolling_actuals)
    print("predictions: ", rolling_predictions)
    ...
    # Plot predictions
    plot_predictions(group_data.set_index('ds'), rolling_predictions, drug_name, factory_name, config.font_path, config.plot_dir)
