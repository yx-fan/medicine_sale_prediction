...
parser.add_argument('--drug_name', type=str, required=True, help='The specific drug name to include for training')
parser.add_argument('--factory_name', type=str, required=True, help='The specific manufacturer name to include for training')
...
# 根据指定的 start_date 进行过滤
df = df[df.index >= start_date_filter]
print(f"训练数据从 {start_date_filter} 开始")

# 根据指定的药品名称和厂家过滤
df = df[(df['药品名称'] == args.drug_name) & (df['厂家'] == args.factory_name)]
print(f"处理的特定药品名称: {args.drug_name}, 厂家: {args.factory_name}")

# 检查是否有足够的数据
if df.empty:
    print(f"没有找到符合条件的药品名称和厂家数据: {args.drug_name} + {args.factory_name}")
    exit()
...
for _, row in unique_groups.iterrows():
    ...
    for t in range(train_end, len(log_y)):      # 使用SARIMAX的预测值加机器学习模型预测残差的混合预测
        ...
        # 确保 predicted_residual 不是 NaN  -- by April
        if np.isnan(predicted_residual):
            print(f"警告: predicted_residual在第 {t} 期的预测中检测到 NaN 残差值，已将其设为 0")
            predicted_residual = 0  # 或者你可以选择其他合适的值，例如前一期的平均残差
        ...
        # 确保 final_pred_log 不是 NaN  -- by April
        if np.isnan(final_pred_log):
            print(f"警告: final_pred_log在第 {t} 期的预测中检测到 NaN 残差值，已将其设为 0")
            final_pred_log= 0  # 或者你可以选择其他合适的值，例如前一期的平均残差
